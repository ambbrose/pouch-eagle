...
export const DesignConfigurator = ({ imageUrl, configId, imageDimensions }: DesignConfiguratorProps) => {
    ...
    async function saveConfiguration() {
        try {
            ...
            toast({
                ...
                description: "There was a problem saving your config, Pleas try again",
                variant: "destructive"
            })
        }
    }
    ...
    function base64ToBlob(base64Data: string, mimeType: string) {
        const byteCharracters = atob(base64Data);
        const byteNumbers = new Array(byteCharracters.length);

        for (let i = 0; i < byteCharracters.length; i++) {
            byteNumbers[i] = byteCharracters.charCodeAt(i);
        }
                                    ...
                                    <div className='mt-3 flex items-center space-x-3'>
                                        {COLORS.map((color) => (
                                            <Field key={color.label}>
                                                <Radio
                                                    value={color}
                                                    className={({ disabled, checked }) => cn(
                                                        'relative flex -m-0.5 items-center justify-center rounded-full p-0.5 active:ring-0 focus:ring-0 active:outline-none focus:outline-none border-2 border-transparent',
                                                        {
